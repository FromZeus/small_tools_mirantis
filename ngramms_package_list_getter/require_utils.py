...
class Require:
  ...
  # Gets all package names and their bound from the line in
  # format: { "pbr" : { (">=", "0.6"), ("<", "0.7"), (">", "0.7"), ("<", "1.0") } }
  @staticmethod
  def parse_line_spec(line):
    res = dict()
    packs = line.split(",")
    for pack_line in packs:
      pack = pack_line.strip()

      name = Require.packageName.search(pack)
      if name:
        name = name.group(0)
        pack = pack[len(name):]
      else:
      ...
      sign = Require.packageEq.search(pack)
      if sign:
        sign = sign.group(0)
        pack = pack[len(sign):]
      else:
      ...
      resVers = Require.packageVers.search(pack)
      if resVers:
        resVers = resVers.group(0)
      else:
        resVers = None

      if name and not res.has_key(name):
        res[name] = set()
      if sign and resVers:
        res[name].add((sign, resVers))
    return res

  # Gets all package names and their bound from whole spec file in
  # format: { "pbr" : { (">=", "0.6"), ("<", "0.7"), (">", "0.7"), ("<", "1.0") } }
  @staticmethod
  def parse_requires_spec(inp):
    res = dict()
    for line in inp:
      if line.startswith("Requires:"):
        res = dict(res.items() + Require.parse_line_spec(line[9:]).items()) # 9 - len("Requires:")
    return res
